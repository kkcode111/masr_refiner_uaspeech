...
class ConformerModelA1(torch.nn.Module):
    def __init__(
        ...
        if if_use_nar:
            ...
            if nar_args is not None:
                nar_kwargs = dict(nar_args)
                nar_kwargs.pop("vocab_size", None)
                nar_kwargs.pop("input_dim", None)
                nar_kwargs.pop("hidden_dim", None)
    ...
    def forward(
        ...
        loss_nar_mlm = torch.tensor(0.0, device=encoder_out.device)
        nar_mlm_token_count = torch.tensor(0.0, device=encoder_out.device)
        nar_mlm_token_ratio = torch.tensor(0.0, device=encoder_out.device)
        if self.ctc_weight != 1.0:
            ...
            loss_nar_mlm = nar_info.get("loss_nar_mlm", loss_nar_mlm)
            nar_mlm_token_count = nar_info.get("nar_mlm_token_count", nar_mlm_token_count)
            nar_mlm_token_ratio = nar_info.get("nar_mlm_token_ratio", nar_mlm_token_ratio)
        else:
            loss_att = None
        # 2b. CTC branch
        ...
        return {
            ...
            "loss_nar_mlm": loss_nar_mlm,
            "nar_mlm_token_count": nar_mlm_token_count,
            "nar_mlm_token_ratio": nar_mlm_token_ratio,
        }
    ...
    def _calc_att_loss(self,
        ...
        if self.nar is not None:
            ...
            loss_att = loss_att + self.nar_ref_weight * nar_res["loss_ref"] + self.nar_error_weight * nar_res["loss_error"]
            if "loss_mlm" in nar_res:
                loss_att = loss_att + self.nar_mlm_weight * nar_res["loss_mlm"]
            nar_info = {
                "loss_nar_ref": nar_res["loss_ref"],
                "loss_nar_error": nar_res["loss_error"],
                "loss_nar_mlm": nar_res.get("loss_mlm", torch.tensor(0.0, device=encoder_out.device)),
                "nar_mlm_token_count": nar_res.get("mlm_token_count", torch.tensor(0.0, device=encoder_out.device)),
                "nar_mlm_token_ratio": nar_res.get("mlm_token_ratio", torch.tensor(0.0, device=encoder_out.device)),
            }
