...
class MASRTrainer(object):
    def __init__(self,
        ...
        :param decoder: 解码器，支持ctc_greedy_search、ctc_prefix_beam_search、attention_rescoring、ctc_beam_search
        :type decoder: str
    ...
    def __train_epoch(self, epoch_id, save_model_path, writer):
        ...
        if isinstance(enable_amp, str):
            enable_amp = enable_amp.lower() == 'true'
        with model_context():
    ...
    def _set_decoder_requires_grad(self, requires_grad: bool):
        m = self.model.module if isinstance(self.model, torch.nn.parallel.DistributedDataParallel) else self.model
    ...
    def _maybe_update_decoder_freeze(self, epoch_id: int):
        ...
        if desired is None:
            return
        if self._decoder_frozen_state is None or self._decoder_frozen_state != desired:
            self._set_decoder_requires_grad(not desired)
            self._decoder_frozen_state = desired
            if self.local_rank == 0:
                if desired:
