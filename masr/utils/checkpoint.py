...
def load_checkpoint(configs, model, optimizer, amp_scaler, scheduler,
    ...
    def load_model_state_error_rate(model_path):
        state_path = os.path.join(model_path, 'model.state')
    ...
    last_model_dir = os.path.join(save_model_path,
                                  ...
                                  'last_model')
    best_model_dir = os.path.join(save_model_path,
                                  f'{configs.model_conf.model}_{save_feature_method}',
                                  'best_model')
    ...
    best_error_rate = load_model_state_error_rate(best_model_dir)
    if best_error_rate is not None:
        error_rate1 = min(error_rate1, best_error_rate)
