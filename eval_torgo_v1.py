...
add_arg("version", str, "v1", "experiment version used in JSON file name")
add_arg("resume_model", str, "torgomodel/v1/ConformerModel_fbank/best_model/",
        "model checkpoint directory or model.pth path")
add_arg("overwrites", str, None, "config overwrites, separated by comma")
add_arg("refiner_weight_grid", str, "0.0",
        "refiner weights for attention_rescoring; v1 baseline should stay at 0.0")
...
if args.save_json:
    ...
    out = {
        ...
        "loss": float(loss) if loss is not None else None,
        "error_result": float(error_result) if error_result is not None else None,
        "best_metric": args.metrics_type,
        "best_metric_value": float(error_result) if error_result is not None else None,
        "best_wer": float(error_result) if args.metrics_type == "wer" and error_result is not None else None,
        "best_refiner_weight": best_refiner_weight,
